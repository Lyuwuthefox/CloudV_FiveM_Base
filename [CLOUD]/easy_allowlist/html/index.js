...
$("#remove-allowlist-btn").click(function() {
	let playerDataModal = $("#player-data-modal");
	
	let identifier = playerDataModal.data("identifier");

	$.post(`https://${resName}/removeAllowlist`, JSON.stringify({identifier: identifier}), function(isSuccessful) {
		if(isSuccessful) {
			playerDataModal.modal("hide");
			loadAllAllowlistedPlayers();
		}
...
$("#remove-queue-priority-btn").click(function() {
	let playerDataModal = $("#player-data-modal");
	...
	$.post(`https://${resName}/removeQueuePriority`, JSON.stringify({identifier: identifier, type: type}), function(isSuccessful) {
		if(isSuccessful) {
			playerDataModal.modal("hide");
...
$("#create-queue-priority-modal").submit(function(event) {
	...
	$.post(`https://${resName}/createQueuePriority`, JSON.stringify({identifier: identifier, priority: priority, type: type}), function(isSuccessful) {
		if(isSuccessful) {
			loadAllQueuePriorities()
		}
	});
...
$("#edit-queue-priority-modal").submit(function(event) {
	if (!this.checkValidity()) {
		event.preventDefault();
		event.stopPropagation();
	...
	let identifier = modal.data("identifier");
	let priority = parseInt( $("#queue-priority-input").val() );
	let type = modal.data("type");

	modal.modal("hide");

	$.post(`https://${resName}/setQueuePriority`, JSON.stringify({identifier: identifier, priority: priority, type: type}), function(isSuccessful) {
		if(isSuccessful) {
			loadAllQueuePriorities();
		} else {
			console.error("Failed to set queue priority");
		}
	});
})
...
$("#delete-reserved-slot-btn").click(function() {
	let editReservedSlotModal = $("#edit-reserved-slot-modal");
	let slotIdentifier = editReservedSlotModal.data("slotIdentifier");

	$.post(`https://${resName}/deleteReservedSlot`, JSON.stringify({
		...
		if(isSuccessful) {
			...
			console.error("Failed to delete reserved slot");
		}
	});
})

$("#edit-reserved-slot-modal").submit(function(event) {
	if (!this.checkValidity()) {
		event.preventDefault();
		event.stopPropagation();
	...
	$.post(`https://${resName}/createReservedSlots`, JSON.stringify({
		identifier: slotIdentifier,
		...
		if(isSuccessful) {
			...
			console.error("Failed to set reserved slot");
		}
	});	
})
...
$("#create-reserved-slots-modal").submit(function(event) {
	if (!this.checkValidity()) {
		event.preventDefault();
		event.stopPropagation();
	...
	$.post(`https://${resName}/createReservedSlots`, JSON.stringify(data), function(isSuccessful) {
		if(isSuccessful) {
