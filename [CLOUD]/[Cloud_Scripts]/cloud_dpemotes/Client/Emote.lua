...
--         if IsInAnimation then
--             -- EmoteCancel()
--             TriggerEvent("ws_notify", "error", "Bug-Using", "Bitte enferne dein Bind mit Unbind Keyboard x", 5000)
--         else
...
function OnEmotePlay(EmoteName)
    ...
    if cooldown then 
      TriggerEvent("sanity:notify", "SYSTEM", "Der Cooldown ist noch aktiv!", 5000, "warning")
        return 
    else
        cooldown = true 
        Citizen.CreateThread(function()
    ...
    if IsPedBeingStunned(ped) or IsPedFalling(ped) or IsPedInMeleeCombat(ped) or IsEntityDead(ped) or IsPedInAnyVehicle(ped, true) then
      TriggerEvent("sanity:notify", "SYSTEM", "Du kannst derzeit kein Emote ausführen", 5000, "error")
        return 
    end
