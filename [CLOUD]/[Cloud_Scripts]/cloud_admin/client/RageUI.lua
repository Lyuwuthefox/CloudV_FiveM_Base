...
if not string.find(Menu.HeaderURL, '.gif') then
    HeaderSprite = 'commonmenu'
    HeaderTexture = 'interaction_bgd'
    HeaderTitle = Menu.Title
else
    local txd = CreateRuntimeTxd('Menu')
    local Header = CreateDui(Menu.HeaderURL, Menu.HeaderWidth, Menu.HeaderHeigth)
...
local RGB = Rainbow(2)
function LoadTheme()
    local LoadThemeKVP = GetResourceKvpInt('cs-admin:Theme')
    if LoadThemeKVP == 0 then
        SetResourceKvpInt('cs-admin:Theme', 1)
        CS_UI.Colours.R = 0
        CS_UI.Colours.G = 0
        CS_UI.Colours.B = 0
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 1 then
        CS_UI.Colours.R = 0
        CS_UI.Colours.G = 0
        CS_UI.Colours.B = 0
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 2 then
        CS_UI.Colours.R = 255
        CS_UI.Colours.G = 0
        CS_UI.Colours.B = 0
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 3 then
        CS_UI.Colours.R = 0
        CS_UI.Colours.G = 123
        CS_UI.Colours.B = 255
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 4 then
        CS_UI.Colours.R = 255
        CS_UI.Colours.G = 255
        CS_UI.Colours.B = 255
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 5 then
        CS_UI.Colours.R = 81
        CS_UI.Colours.G = 0
        CS_UI.Colours.B = 255
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 6 then
        CS_UI.Colours.R = 196
        CS_UI.Colours.G = 118
        CS_UI.Colours.B = 0
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 7 then
        CS_UI.Colours.R = 255
        CS_UI.Colours.G = 242
        CS_UI.Colours.B = 0
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 8 then
        CS_UI.Colours.R = 3
        CS_UI.Colours.G = 252
        CS_UI.Colours.B = 15
        CS_UI.Colours.A = 255
    elseif LoadThemeKVP == 9 then
        rainbow_active = true
    end
end
function SaveTheme(Index)
    SetResourceKvpInt('cs-admin:Theme', Index)
end
function ForceTheme(Index)
    if Index == 1 then
        SaveTheme(1)
...
RageUI.Settings = {
    ...
    Audio = {
        Id = nil,
        UpDown = {
            audioName = Menu.Sounds.UpDownSound,
            audioRef = Menu.Sounds.UpDownSoundRef,
        },
        LeftRight = {
            audioName = Menu.Sounds.LeftRightSound,
            audioRef = Menu.Sounds.LeftRightSoundRef,
        },
        Select = {
            audioName = Menu.Sounds.SelectSound,
            audioRef = Menu.Sounds.SelectSoundRef,
        },
        Back = {
            audioName = Menu.Sounds.BackSound,
            audioRef = Menu.Sounds.BackSoundRef,
        },
        Error = {
            audioName = Menu.Sounds.ErrorSound,
            audioRef = Menu.Sounds.ErrorSoundRef,
        },
        Slider = {
...
local RGB = Rainbow(2)
Citizen.CreateThread(function() --maybe gets updated later but ye dont know tak tak men [el game timer]
    while true do
        Citizen.Wait(255)
        while rainbow_active do
            Citizen.Wait(0)
            RGB = Rainbow(2)
            CS_UI.Colours.R = RGB.r
            CS_UI.Colours.G = RGB.g
            CS_UI.Colours.B = RGB.y
            CS_UI.Colours.A = 125
        end
    end
end)
