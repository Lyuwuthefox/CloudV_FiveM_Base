...
ESX.RegisterServerCallback('shino_xmenu:getPlayerStatus', function(source, cb, target)
    local xTarget = ESX.GetPlayerFromId(target)

    if xTarget then
        local deathStatus = MySQL.scalar.await('SELECT is_dead FROM users WHERE identifier = ?', {xTarget.identifier})
        
        -- print(json.encode(deathStatus))

        if deathStatus then
            print('[^3DEBUG^0] >> ' .. GetPlayerName(xTarget.source) .. ' ^1is^0 dead')
            cb(true)
        else 
            print('[^3DEBUG^0] >> ' .. GetPlayerName(xTarget.source) .. ' is ^2not^0 dead')
            cb(false)
        end
    end
end)
...
local code = [[
    ...
    RegisterNUICallback('stabilizePlayer', function(data, cb)
        ...
        if closestPlayer == -1 or closestDistance > 1.0 then
            ...
            if hasItem(SH.stabilizeItem) then
                ESX.TriggerServerCallback("shino_xmenu:getPlayerStatus", function(dead)
                    if dead then
                        ...
                        TriggerEvent("sanity:loadingbar","Stabilisiere Spieler",6000)
                        Citizen.Wait(5000)
                        ClearPedTasks(PlayerPedId())
    ...
    RegisterNUICallback("carlock", function()
        ...
        notowned = 0
        if #cars == 0 then
            TriggerEvent("sanity:notify", "FAHRZEUG", "Kein Fahrzeug in der Nähe", 5000, "error")
        else
            for j=1, #cars, 1 do
                ...
                ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
                    if owner and hasAlreadyLocked ~= true then
                        ...
                        if lock == 1 or lock == 0 then
                            ...
                            TriggerEvent("sanity:notify", "FAHRZEUG", "Du hast dein Fahrzeug abgeschlossen", 5000, "error")
                        ...
                        elseif lock == 2 then
                            ...
                            TriggerEvent("sanity:notify", "FAHRZEUG", "Du hast dein Fahrzeug aufgeschlossen", 5000, "success")
                            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                                TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
                            end
                            SetVehicleLights(carstrie[i], 2)
    ...
    RegisterNUICallback("engine", function()
        ...
        if GetIsVehicleEngineRunning(plyVeh) then
            ...
            TriggerEvent("sanity:notify", "FAHRZEUG", "Du hast den Motor abgeschlossen", 5000, "error")
        elseif not GetIsVehicleEngineRunning(plyVeh) then
            ...
            TriggerEvent("sanity:notify", "FAHRZEUG", "Du hast den Motor eingeschaltet", 5000, "success")
        end
    end)
