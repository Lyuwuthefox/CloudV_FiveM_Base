...
RegisterNetEvent('esx_policejob:giveLoadout', function(id)
	local src = source
	...
	for i = 1, #Config.Loadouts[id].content do
		if Config.Loadouts[id].content[i].type == 'weapon' then
			xPlayer.addWeapon(Config.Loadouts[id].content[i].name, Config.Loadouts[id].content[i].ammo)
		elseif Config.Loadouts[id].content[i].type == 'item' then
			xPlayer.addInventoryItem(Config.Loadouts[id].content[i].name, Config.Loadouts[id].content[i].amount)
		else
			print('[^3DEBUG] Invalid Loadout Type: ' .. Config.Loadouts[id].content[i].type)
		end
	end
end)

RegisterNetEvent('esx_policejob:refillAmmo', function(weaponLabel)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	...
	xPlayer.addWeaponAmmo(weaponLabel, Config.RefillAmmoAmount)
end)