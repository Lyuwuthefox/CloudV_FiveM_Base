...
for i = 1, #Weapons do
	weaponHashes[GetHashKey(Weapons[i])] = Weapons[i]
	-- print('Key: ' .. GetHashKey(Weapons[i]) .. ' Value: ' .. Weapons[i])
	-- print(weaponHashes[GetHashKey(Weapons[i])])
end

function getWeaponName(weaponHash)
	local weaponHash = tonumber(weaponHash)
	local weaponName = weaponHashes[weaponHash]
	return weaponName;
end
...
function OpenArmoryMenu(station)
	...
	if Config.OxInventory then
		...
		elements = {
			{label = "Munition nachfüllen", value = 'add_ammo'}
		}

		for k, v in pairs(Config.Loadouts) do
			table.insert(elements, {label = v.label, value = 'loadout_' .. k})
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		elseif data.current.value == 'get_stock' then
			OpenGetStocksMenu()
		elseif data.current.value == 'add_ammo' then
			if IsPedArmed(PlayerPedId(), 4) then
				local weaponHash = GetSelectedPedWeapon(PlayerPedId())
				local weaponLabel = getWeaponName(weaponHash)
				TriggerServerEvent('esx_policejob:refillAmmo', tostring(weaponLabel))
			else
				TriggerEvent("sanity:notify", "Waffenkammer", "Du hast keine Waffe in der Hand", 5000, "error")
			end
		end

		for k, v in pairs(Config.Loadouts) do
			if data.current.value == 'loadout_' .. k then
				TriggerServerEvent('esx_policejob:giveLoadout', tonumber(k))
			end
		end
