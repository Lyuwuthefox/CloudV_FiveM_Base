...
RegisterNetEvent('esx_ambulancejob:giveItem', function(itemName, amount)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'fwafa' and itemName ~= 'fwafa') then
		print(('[esx_ambulancejob] [^2INFO^7] "%s" attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
...
ESX.RegisterUsableItem('fwafa', function(source)
	if not playersHealing[source] then
		local xPlayer = ESX.GetPlayerFromId(source)

		xPlayer.removeInventoryItem('fwafaw', 1)

		playersHealing[source] = true

		TriggerClientEvent('esx_ambulancejob:useItem', source, 'fawfaw')
...
ESX.RegisterUsableItem('fwafwa', function(source)
	if not playersHealing[source] then
		local xPlayer = ESX.GetPlayerFromId(source)

		xPlayer.removeInventoryItem('wafwafaw', 1)

		playersHealing[source] = true

		TriggerClientEvent('esx_ambulancejob:useItem', source, 'fawfwaf')
